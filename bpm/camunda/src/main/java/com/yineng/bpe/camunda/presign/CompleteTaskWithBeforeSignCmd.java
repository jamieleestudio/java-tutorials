...
public class CompleteTaskWithBeforeSignCmd implements Command<Void> {
    ...
    public Void execute(CommandContext commandContext) {
        ...
        if (modeObj == null || totalObj == null || requiredObj == null) {
            ...
            BeforeSignService.BeforeSignMode mode;
            try {
                mode = BeforeSignService.BeforeSignMode.valueOf(modeName);
            } catch (IllegalArgumentException ex) {
                mode = BeforeSignService.BeforeSignMode.ALL;
            }
            boolean pass;
            if (mode == BeforeSignService.BeforeSignMode.ALL) {
                pass = open == 0;
            } else if (mode == BeforeSignService.BeforeSignMode.ANY) {
                pass = finished >= 1;
            } else {
